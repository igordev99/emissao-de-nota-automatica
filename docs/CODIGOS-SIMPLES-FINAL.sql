-- ================================================
-- VERSÃO SUPER SIMPLES - APENAS CÓDIGOS DE REFERÊNCIA
-- Execute no SQL Editor do Supabase
-- Esta versão ignora completamente as validações de faixa
-- ================================================
...
BEGIN
    ...
    RAISE NOTICE '👤 Usando User ID: %', my_user_id;
    
    -- Criar grupo para códigos
    INSERT INTO formula_groups (name, description, user_id, created_at, updated_at)
    VALUES ('📋 Códigos de Serviço Municipal', 'Lista de referência dos códigos de serviços - não calculam faixas', my_user_id, NOW(), NOW())
    RETURNING id INTO my_group_id;
    
    RAISE NOTICE '📁 Grupo criado com ID: %', my_group_id;
    
    -- Desabilitar TODAS as validações temporariamente
    ALTER TABLE formula_rows DISABLE TRIGGER ALL;
    
    -- Inserir todos os códigos de uma vez
    ...
    RAISE NOTICE '🎉 SUCESSO! Todos os 60 códigos foram inseridos!';
    RAISE NOTICE '📊 Grupo: "📋 Códigos de Serviço Municipal"';
    RAISE NOTICE '🔍 Use a busca por código para encontrar rapidamente';
    
END $$;

-- ================================
-- VERIFICAÇÕES E RELATÓRIO FINAL
-- ================================

-- Mostrar resumo da importação
SELECT 
    '🎯 IMPORTAÇÃO FINALIZADA' as status,
...
WHERE fg.name = '📋 Códigos de Serviço Municipal';

-- Mostrar primeiros 10 códigos para conferência
SELECT 
    '🔢 ' || fr.codigo_servico as "Código",
    LEFT(fr.descricao_servico, 60) || '...' as "Descrição (Resumida)",
    CASE WHEN fr.iss_retido_das THEN '✅ Sim' ELSE '❌ Não' END as "ISS Retido",
    ROUND(fr.indice * 100, 2) || '%' as "Alíquota"
FROM formula_rows fr
JOIN formula_groups fg ON fr.group_id = fg.id
WHERE fg.name = '📋 Códigos de Serviço Municipal'
