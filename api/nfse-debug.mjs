// Função Vercel com debug de autenticação JWT
...
async function createNfseApp() {
  ...
  console.log(`JWT_SECRET configured: ${!!jwtSecret}, length: ${jwtSecret?.length}`);
  
  await app.register(jwt, { secret: jwtSecret });

  // Helper para autenticação com debug
  const authenticate = async (request) => {
    try {
      console.log('Auth headers:', request.headers.authorization);
      await request.jwtVerify();
      console.log('JWT verified successfully, user:', request.user);
    } catch (error) {
  ...
  app.post('/auth/debug', async (request) => {
    try {
